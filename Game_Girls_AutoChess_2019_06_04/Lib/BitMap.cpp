...
namespace JEngine
{
	...
	void BitMap::Draw(int x, int y, int LastX, int LastY, int MemX, int MemY)
	{
		AdjustAnchorPoint(x, y);
		TransparentBlt(ResoucesManager::GetInstance()->GetBackDC(), x, y, LastX, LastY, m_hMemDC, MemX, MemY, LastX, LastY, RGB(255, 0, 255));
	}

	void BitMap::Draw(int x, int y, int LastX, int LastY)
	{
		AdjustAnchorPoint(x, y);
		TransparentBlt(ResoucesManager::GetInstance()->GetBackDC(), x, y, LastX, LastY, m_hMemDC, 0, 0, m_size.cx, m_size.cy, RGB(255, 0, 255));
	}
	...
	void BitMap::Draw(JEngine::POINT pt, JEngine::POINT Lastpt, JEngine::POINT Mempt)
	{
		Draw(pt.x, pt.y, Lastpt.x, Lastpt.y, Mempt.x, Mempt.y);
	}

	void BitMap::Draw(JEngine::POINT pt, JEngine::POINT Lastpt)
	{
		Draw(pt.x, pt.y, Lastpt.x, Lastpt.y);
	}
}